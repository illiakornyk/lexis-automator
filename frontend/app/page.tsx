...
import { Search, Download, FileAudio, RefreshCw, CheckCircle2, BookOpen } from "lucide-react";
        ...
        {!isLoading && wordData && (
          <>
            ...
            <div>
              ...
              {/* Find the first phonetic text available */}
              {wordData.phonetics?.find(p => p.text)?.text && (
                <p className="text-slate-500 mt-1">{wordData.phonetics.find(p => p.text)?.text}</p>
              )}
            </div>
